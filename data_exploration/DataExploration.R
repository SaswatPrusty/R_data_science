...
#setwd("~/Dropbox/MIS 6356 BA with R/--R/Data Exploration")
...
#without using sapply, calculate the mean of MEDV
#

mean_MEDV <- mean(housing.df$MEDV)

var1 <- housing.df$MEDV
str(var1)
...
## boxplot of MEDV for different values of CHAS
boxplot(housing.df$MEDV ~ housing.df$RAD, xlab = "CHAS", ylab = "MEDV")
...
library(ggplot2)
ggplot(housing.df) + geom_point(aes(x = LSTAT, y = MEDV), colour = "navy", alpha = 0.2)
